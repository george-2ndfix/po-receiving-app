...
# ============================================
# Label PDF Generation for QL-810W
# ============================================
@app.route('/api/label-pdf', methods=['POST'])
@login_required
def generate_label_pdf():
    """Generate label PDF for QL-810W with DK-2225 38mm tape.
    Accepts items array and returns a downloadable PDF."""
    ...
    def auto_shrink(canvas_obj, text, font, max_fs, max_w):
        fs = max_fs
        while canvas_obj.stringWidth(text, font, fs) > max_w and fs > 4:
            fs -= 0.5
        return fs
    ...
    if orientation == 'portrait':
        ...
        TEXT_H = PAGE_W - 2 * MARGIN

        for item in items:
            c.saveState()
            c.translate(PAGE_W - MARGIN, MARGIN)
            c.rotate(90)
            ...
            line1 = f"Job {item.get('jobNumber', '')}  \u00b7  {item.get('customerName', '')}"
            fs = auto_shrink(c, line1, BOLD, 11, TEXT_W)
            c.setFont(BOLD, fs)
            c.drawString(0, y_top, line1)

            line2 = f"{item.get('partNo', '')}  \u00b7  {item.get('description', '')}"
            fs = auto_shrink(c, line2, REGULAR, 9, TEXT_W)
            c.setFont(REGULAR, fs)
            c.drawString(0, y_mid, line2)

            parts = [f"Qty: {item.get('quantity', 0)}"]
            sl = item.get('storageLocation', '')
            if sl:
                parts.append(sl)
            parts.append(today)
            parts.append(f"PO {po_number}")
            line3 = "  \u00b7  ".join(parts)
            fs = auto_shrink(c, line3, REGULAR, 8, TEXT_W)
            c.setFont(REGULAR, fs)
            c.drawString(0, y_bot, line3)
        ...
        for item in items:
            ...
            fs = auto_shrink(c, line1, BOLD, 11, TEXT_W)
            c.setFont(BOLD, fs)
            c.drawString(x, y1, line1)

            line2 = f"{item.get('partNo', '')}  \u00b7  {item.get('description', '')}"
            fs = auto_shrink(c, line2, REGULAR, 9, TEXT_W)
            c.setFont(REGULAR, fs)
            c.drawString(x, y2, line2)
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f'labels-PO{po_number}-{orientation}.pdf'
    )


@app.route('/api/test-label-pdf')
def test_label_pdf():
    """Generate test label PDF for QL-810W testing (no login required)."""
    from reportlab.pdfgen import canvas as pdf_canvas
    from reportlab.lib.units import mm
    ...
    test_items = [
        {'jobNumber': '5364', 'customerName': 'LEE', 'partNo': 'SPM-2040-720',
         'description': 'Colonial 4 panel 2040x720x35 SPM door', 'quantity': 2, 'storageLocation': 'Customer Cupboard'},
        {'jobNumber': '5401', 'customerName': "O'BRIEN FAMILY TRUST", 'partNo': 'ABC-123',
         'description': 'Solid core door 2040x820 primed', 'quantity': 5, 'storageLocation': 'Rack B'},
        {'jobNumber': '5380', 'customerName': 'WU', 'partNo': 'HNG-SS-100',
         'description': 'Hinge stainless steel 100mm fixed pin', 'quantity': 12, 'storageLocation': 'Shelf 3'},
        {'jobNumber': '5422', 'customerName': 'JOHNSON & ASSOCIATES PTY LTD', 'partNo': 'LCK-MRT-60',
         'description': 'Mortice lock 60mm backset satin chrome', 'quantity': 1, 'storageLocation': 'Lock Cabinet'},
        {'jobNumber': '5399', 'customerName': 'SMITH', 'partNo': 'FRM-2340-120',
         'description': 'Door frame set pine 2340x120 rebated', 'quantity': 3, 'storageLocation': 'Timber Rack'},
    ]
    po_number = '21027'
    today = datetime.now().strftime('%d/%m/%Y')
    ...
    if orientation == 'portrait':
        ...
        for item in test_items:
            ...
            line1 = f"Job {item['jobNumber']}  \u00b7  {item['customerName']}"
            fs = auto_shrink(c, line1, BOLD, 11, TEXT_W)
            c.setFont(BOLD, fs)
            c.drawString(0, y_top, line1)

            line2 = f"{item['partNo']}  \u00b7  {item['description']}"
            fs = auto_shrink(c, line2, REGULAR, 9, TEXT_W)
            c.setFont(REGULAR, fs)
            c.drawString(0, y_mid, line2)

            parts = [f"Qty: {item['quantity']}"]
            if item.get('storageLocation'):
                parts.append(item['storageLocation'])
            parts.append(today)
            parts.append(f"PO {po_number}")
            line3 = "  \u00b7  ".join(parts)
            fs = auto_shrink(c, line3, REGULAR, 8, TEXT_W)
    ...
    return send_file(
        ...
        mimetype='application/pdf',
        as_attachment=True,
        download_name=f'test-labels-{orientation}.pdf'
    )
